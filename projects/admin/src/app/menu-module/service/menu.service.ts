  ...
  public generateMenuLanguages(): MenuItem[] {
    ...
    this.configService.languages.map((language: string) => {
      languagesMenu.push({
        ...
        command: () => this.httpClient.post(`/language`, {lang: language}).subscribe(() => this.translateService.use(language))
      });
    });
    ...
    return this.libraryApiService.findByLibrariesPidAndOrderBy$(librariesPid).pipe(
      ...
      map((libraries: any) => {
        ...
        if (!this.librarySwitchDataStorage.has()) {
          ...
          this.librarySwitchDataStorage.save({
            userId: this.userService.user.id,
            currentLibrary: libraryActive.metadata.pid,
            libraryName: libraryActive.metadata.name
          });
        } else {
