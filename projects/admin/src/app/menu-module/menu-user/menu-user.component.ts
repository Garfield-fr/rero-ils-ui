...
import { MENU_USER } from '../definition/menu-user';
...
export class MenuUserComponent implements OnInit, OnDestroy {
  ...
  private menuTranslateService = inject(MenuTranslateService);
  ...
  ngOnInit(): void {
    ...
    this.subscription.add(
      this.menuService.generateMenuLibrary$().subscribe((menu: object) => this.items = [menu, ...this.items])
    );
    this.subscription.add(
